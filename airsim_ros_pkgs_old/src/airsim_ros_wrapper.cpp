...
void AirsimROSWrapper::initialize_airsim() {
  ...
  try {
    if (airsim_mode_ == AIRSIM_MODE::DRONE) {
      ...
      airsim_client_ = std::move(std::unique_ptr<msr::airlib::RpcLibClientBase>(
...
void AirsimROSWrapper::create_ros_pubs_from_settings_json() {
  ...
  rpy_thrust_sub_ = nh_private_.subscribe(
      "/setpoint_raw/attitude", 50, &AirsimROSWrapper::rpy_thrust_cmd_cb, this,
  ...
  debug_pub_ = nh_private_.advertise<std_msgs::Float32>("/debug_msg", 10);
  ...
  for (const auto& curr_vehicle_elem : AirSimSettings::singleton().vehicles) {
    ...
    vehicle_ros->odom_local_pub = nh_private_.advertise<nav_msgs::Odometry>(
        curr_vehicle_name + "/" + odom_frame_id_, 1);

    vehicle_ros->env_pub = nh_private_.advertise<airsim_ros_pkgs::Environment>(
        curr_vehicle_name + "/environment", 10);

    vehicle_ros->global_gps_pub = nh_private_.advertise<sensor_msgs::NavSatFix>(
        curr_vehicle_name + "/global_gps", 10);
    ...
    if (airsim_mode_ == AIRSIM_MODE::DRONE) {
      ...
      // vehicle name it was by passing curr_vehicle_name as the 2nd argument
      drone->vel_cmd_body_frame_sub =
          nh_private_.subscribe<airsim_ros_pkgs::VelCmd>(
              curr_vehicle_name + "/vel_cmd_body_frame", 1,
              boost::bind(
                  &AirsimROSWrapper::vel_cmd_body_frame_cb, this, _1,
                  vehicle_ros
                      ->vehicle_name));  // todo
                                         // ros::TransportHints().tcpNoDelay();
      drone->vel_cmd_world_frame_sub =
          nh_private_.subscribe<airsim_ros_pkgs::VelCmd>(
      ...
      car->car_cmd_sub = nh_private_.subscribe<airsim_ros_pkgs::CarControls>(
          curr_vehicle_name + "/car_cmd", 1,
          boost::bind(&AirsimROSWrapper::car_cmd_cb, this, _1,
                      vehicle_ros->vehicle_name));
      car->car_state_pub = nh_private_.advertise<airsim_ros_pkgs::CarState>(
          curr_vehicle_name + "/car_state", 10);
    }
    ...
    for (auto& curr_camera_elem : vehicle_setting->cameras) {
      ...
      for (const auto& curr_capture_elem : camera_setting.capture_settings) {
        ...
        // AirSimSettings::initializeCaptureSettings() which initializes default
        // capture settings for _all_ NINE
        ...
        if (!(std::isnan(capture_setting.fov_degrees))) {
          ...
          // if {DepthPlanner, DepthPerspective,DepthVis, DisparityNormalized},
          // get float image
          else {
            current_image_request_vec.push_back(
        ...
        switch (sensor_setting->sensor_type) {
          case SensorBase::SensorType::Barometer: {
            ...
            sensor_publisher.publisher =
                nh_private_.advertise<airsim_ros_pkgs::Altimeter>(
          ...
          case SensorBase::SensorType::Magnetometer: {
            ...
            sensor_publisher.publisher =
                nh_private_.advertise<sensor_msgs::MagneticField>(
          ...
          case SensorBase::SensorType::Distance: {
            ...
            sensor_publisher.publisher =
                nh_private_.advertise<sensor_msgs::Range>(
          ...
          case SensorBase::SensorType::Lidar: {
            ...
            append_static_lidar_tf(vehicle_ros.get(), sensor_name,
                                   lidar_setting);
            sensor_publisher.publisher =
    ...
    // we want fast access to the lidar sensors for callback handling, sort them
    // out now
    auto isLidar = std::function<bool(const SensorPublisher& pub)>(
        [](const SensorPublisher& pub) {
    ...
    vehicle_name_ptr_map_.emplace(
        curr_vehicle_name,
        std::move(vehicle_ros));  // allows fast lookup in command callbacks in
                                  // case of a lot of drones
  }
  ...
  if (vehicle_name_ptr_map_.size() > 1 && airsim_mode_ == AIRSIM_MODE::DRONE) {
    ...
    // gimbal_angle_quat_cmd_sub_ = nh_.subscribe("gimbal_angle_quat_cmd", 50,
    // &AirsimROSWrapper::gimbal_angle_quat_cmd_cb, this);

    vel_cmd_all_body_frame_sub_ = nh_private_.subscribe(
        "all_robots/vel_cmd_body_frame", 1,
        &AirsimROSWrapper::vel_cmd_all_body_frame_cb, this);
    vel_cmd_all_world_frame_sub_ = nh_private_.subscribe(
        "all_robots/vel_cmd_world_frame", 1,
        &AirsimROSWrapper::vel_cmd_all_world_frame_cb, this);

    vel_cmd_group_body_frame_sub_ = nh_private_.subscribe(
        "group_of_robots/vel_cmd_body_frame", 1,
        &AirsimROSWrapper::vel_cmd_group_body_frame_cb, this);
    vel_cmd_group_world_frame_sub_ = nh_private_.subscribe(
        "group_of_obots/vel_cmd_world_frame", 1,
        &AirsimROSWrapper::vel_cmd_group_world_frame_cb, this);

    takeoff_group_srvr_ = nh_private_.advertiseService(
        "group_of_robots/takeoff", &AirsimROSWrapper::takeoff_group_srv_cb,
        this);
...
void AirsimROSWrapper::car_cmd_cb(
    const airsim_ros_pkgs::CarControls::ConstPtr& msg,
    const std::string& vehicle_name) {
  std::lock_guard<std::mutex> guard(drone_control_mutex_);
...
void AirsimROSWrapper::vel_cmd_body_frame_cb(
  ...
  auto drone =
      static_cast<MultiRotorROS*>(vehicle_name_ptr_map_[vehicle_name].get());
  ...
  drone->vel_cmd.x =
      (msg->twist.linear.x * cos(yaw)) -
      (msg->twist.linear.y * sin(yaw));  // body frame assuming zero pitch roll
...
void AirsimROSWrapper::vel_cmd_all_body_frame_cb(
  ...
  for (auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_) {
    auto drone =
        static_cast<MultiRotorROS*>(vehicle_name_ptr_pair.second.get());
...
void AirsimROSWrapper::gimbal_angle_quat_cmd_cb(
  ...
  try {
    ...
    gimbal_cmd_.target_quat =
        get_airlib_quat(quat_control_cmd);  // airsim uses wxyz
...
// hzc position cmd callback
void AirsimROSWrapper::rpy_thrust_cmd_cb(
    const mavros_msgs::AttitudeTarget& attitude_cmd_msg) {
  // Eigen::Quaterniond cmd_quater;
  // cmd_quater.w() = attitude_cmd_msg.orientation.w;
  // cmd_quater.x() = attitude_cmd_msg.orientation.x;
  // cmd_quater.y() = attitude_cmd_msg.orientation.y;
  // cmd_quater.z() = attitude_cmd_msg.orientation.z;
  // double thrust = attitude_cmd_msg.thrust;
  // double yaw,pitch,roll;
  // cmd_quater = cmd_quater.normalized();
  // roll = atan2(2 * (cmd_quater.w() * cmd_quater.x() + cmd_quater.y() *
  // cmd_quater.z()), 1 - 2 * (cmd_quater.x() * cmd_quater.x() + cmd_quater.y()
  // * cmd_quater.y())); pitch = asin(2 * (cmd_quater.w() * cmd_quater.y() -
  // cmd_quater.z() * cmd_quater.x())); yaw = atan2(2 * (cmd_quater.w() *
  // cmd_quater.z() + cmd_quater.x() * cmd_quater.y()), 1 - 2 * (cmd_quater.y()
  // * cmd_quater.y() + cmd_quater.z() * cmd_quater.z())); static int a = 0;
  if (isENU_) {
    // a++;
    // //
    // static_cast<msr::airlib::MultirotorRpcLibClient*>(airsim_client_.get())->moveByRollPitchYawThrottleAsync(roll,-pitch,M_PI_2-yaw,thrust,0.005,"drone_1");
    // if(a<1000) {
    ...
    // -attitude_cmd_msg.body_rate.y,-attitude_cmd_msg.body_rate.z,0.16,0.005);
    // }else{
    //     static_cast<msr::airlib::MultirotorRpcLibClient*>(airsim_client_.get())->moveByAngleRatesThrottleAsync(attitude_cmd_msg.body_rate.x,
    // -attitude_cmd_msg.body_rate.y,-attitude_cmd_msg.body_rate.z,0.11736,0.005);
    // }

    double thrust = attitude_cmd_msg.thrust;
    static_cast<msr::airlib::MultirotorRpcLibClient*>(airsim_client_.get())
        ->moveByAngleRatesThrottleAsync(
            attitude_cmd_msg.body_rate.x, -attitude_cmd_msg.body_rate.y,
            -attitude_cmd_msg.body_rate.z, thrust, 0.01);
    ...
    static_cast<msr::airlib::MultirotorRpcLibClient*>(airsim_client_.get())
        ->moveByAngleRatesThrottleAsync(
            attitude_cmd_msg.body_rate.x, attitude_cmd_msg.body_rate.y,
            attitude_cmd_msg.body_rate.z, attitude_cmd_msg.thrust, 0.01);
  }
  return;  // hzchzchzc
}
// todo support multiple gimbal commands
...
airsim_ros_pkgs::CarState AirsimROSWrapper::get_roscarstate_msg_from_car_state(
...
nav_msgs::Odometry AirsimROSWrapper::get_odom_msg_from_car_state(
    const msr::airlib::CarApiBase::CarState& car_state) const {
  ...
  odom_msg.twist.twist.linear.x =
      car_state.kinematics_estimated.twist.linear.x();
  odom_msg.twist.twist.linear.y =
      car_state.kinematics_estimated.twist.linear.y();
  odom_msg.twist.twist.linear.z =
      car_state.kinematics_estimated.twist.linear.z();
  odom_msg.twist.twist.angular.x =
      car_state.kinematics_estimated.twist.angular.x();
  odom_msg.twist.twist.angular.y =
      car_state.kinematics_estimated.twist.angular.y();
  odom_msg.twist.twist.angular.z =
      car_state.kinematics_estimated.twist.angular.z();
...
nav_msgs::Odometry AirsimROSWrapper::get_odom_msg_from_multirotor_state(
  ...
  if (isENU_) {
    ...
    // std::cout<<"w: "<<enuq.w()<<" x: "<<enuq.x()<<" y: "<<enuq.y()<<" z:
    // "<<enuq.z();
    ...
    // roll = atan2(2 * (attitude_quater.w() * attitude_quater.x() +
    // attitude_quater.y() * attitude_quater.z()), 1 - 2 * (attitude_quater.x()
    // * attitude_quater.x() + attitude_quater.y() * attitude_quater.y()));
    // pitch = asin(2 * (attitude_quater.w() * attitude_quater.y() -
    // attitude_quater.z() * attitude_quater.x())); yaw = atan2(2 *
    // (attitude_quater.w() * attitude_quater.z() + attitude_quater.x() *
    // attitude_quater.y()), 1 - 2 * (attitude_quater.y() * attitude_quater.y()
    // + attitude_quater.z() * attitude_quater.z()));
    // //NED
...
nav_msgs::Odometry AirsimROSWrapper::get_odom_msg_from_truth_state(
    const msr::airlib::Kinematics::State& truth_state) const {
  nav_msgs::Odometry odom_msg;
  ...
  if (isENU_) {
    ...
    // std::cout<<"----------------------------------------\n";
    // std::cout<<rota<<"\n";
    // std::cout<<"111Enu: "<<"w: "<<enuq.w()<<" x: "<<enuq.x()<<" y:
    // "<<enuq.y()<<" z: "<<enuq.z()<<"\n"; std::cout<<"rot:
    // \n"<<rota<<std::endl;
    odom_msg.pose.pose.orientation.w = enuq.w();
    odom_msg.pose.pose.orientation.x = enuq.x();
  ...
  if (isENU_) {
    try {
      ...
      auto transformStampedENU = tf_buffer_.lookupTransform(
          AIRSIM_FRAME_ID, vehicle_name, ros::Time(0), ros::Duration(1));
...
airsim_ros_pkgs::Environment AirsimROSWrapper::get_environment_msg_from_airsim(
...
sensor_msgs::MagneticField AirsimROSWrapper::get_mag_msg_from_airsim(
...
// todo covariances
sensor_msgs::NavSatFix AirsimROSWrapper::get_gps_msg_from_airsim(
    const msr::airlib::GpsBase::Output& gps_data) const {
...
sensor_msgs::Range AirsimROSWrapper::get_range_from_airsim(
...
void AirsimROSWrapper::drone_state_timer_cb(const ros::TimerEvent& event) {
  try {
    ...
    // compatibility if (!airsim_client_->simIsPaused())
    // {
    //     // airsim_client needs to provide the simulation time in a future
    //     version of the API ros_clock_.clock = now;
    // }
...
void AirsimROSWrapper::update_and_publish_static_transforms(
...
ros::Time AirsimROSWrapper::update_state() {
  ...
  for (auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_) {
    ...
    if (airsim_mode_ == AIRSIM_MODE::DRONE) {
      auto drone = static_cast<MultiRotorROS*>(vehicle_ros.get());
      auto rpc = static_cast<msr::airlib::MultirotorRpcLibClient*>(
          airsim_client_.get());
      drone->curr_drone_state =
          rpc->getMultirotorState(vehicle_ros->vehicle_name);
      drone->truth_state =
          rpc->simGetGroundTruthKinematics(vehicle_ros->vehicle_name);
      ...
      vehicle_ros->curr_odom =
          get_odom_msg_from_truth_state(drone->truth_state);

    } else {
      auto car = static_cast<CarROS*>(vehicle_ros.get());
      auto rpc =
          static_cast<msr::airlib::CarRpcLibClient*>(airsim_client_.get());
    ...
    airsim_ros_pkgs::Environment env_msg =
        get_environment_msg_from_airsim(env_data);
...
void AirsimROSWrapper::publish_vehicle_state() {
  for (auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_) {
    ...
    // std_msgs::Float32 a;
    // a.data = vehicle_ros->curr_odom.pose.pose.position.x;
    ...
    for (auto& sensor_publisher : vehicle_ros->sensor_pubs) {
      switch (sensor_publisher.sensor_type) {
        case SensorBase::SensorType::Barometer: {
          auto baro_data = airsim_client_->getBarometerData(
              sensor_publisher.sensor_name, vehicle_ros->vehicle_name);
          airsim_ros_pkgs::Altimeter alt_msg =
              get_altimeter_msg_from_airsim(baro_data);
        ...
        case SensorBase::SensorType::Distance: {
          auto distance_data = airsim_client_->getDistanceSensorData(
              sensor_publisher.sensor_name, vehicle_ros->vehicle_name);
        ...
        case SensorBase::SensorType::Gps: {
          auto gps_data = airsim_client_->getGpsData(
              sensor_publisher.sensor_name, vehicle_ros->vehicle_name);
        ...
        case SensorBase::SensorType::Magnetometer: {
          auto mag_data = airsim_client_->getMagnetometerData(
              sensor_publisher.sensor_name, vehicle_ros->vehicle_name);
          sensor_msgs::MagneticField mag_msg =
              get_mag_msg_from_airsim(mag_data);
...
void AirsimROSWrapper::update_commands() {
  for (auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_) {
    ...
    if (airsim_mode_ == AIRSIM_MODE::DRONE) {
      ...
      // {
      //     std::lock_guard<std::mutex> guard(drone_control_mutex_);
      //     static_cast<msr::airlib::MultirotorRpcLibClient*>(airsim_client_.get())->moveByVelocityAsync(drone->vel_cmd.x,
      //     drone->vel_cmd.y, drone->vel_cmd.z, vel_cmd_duration_,
      //         msr::airlib::DrivetrainType::MaxDegreeOfFreedom,
      //         drone->vel_cmd.yaw_mode, drone->vehicle_name);
      // }
      // drone->has_vel_cmd = false;
      static_cast<msr::airlib::MultirotorRpcLibClient*>(airsim_client_.get())
          ->moveByRollPitchYawThrottleAsync(0, 0, 0, 0.65, 0.025, "drone_1");
    } else {
      // send control commands from the last callback to airsim
      auto car = static_cast<CarROS*>(vehicle_ros.get());
      if (car->has_car_cmd) {
        std::lock_guard<std::mutex> guard(drone_control_mutex_);
        static_cast<msr::airlib::CarRpcLibClient*>(airsim_client_.get())
            ->setCarControls(car->car_cmd, vehicle_ros->vehicle_name);
      }
  ...
  if (has_gimbal_cmd_) {
    std::lock_guard<std::mutex> guard(drone_control_mutex_);
    airsim_client_->simSetCameraPose(
...
void AirsimROSWrapper::append_static_vehicle_tf(
    VehicleROS* vehicle_ros, const VehicleSetting& vehicle_setting) {
  ...
  if (isENU_) {
    std::swap(vehicle_tf_msg.transform.translation.x,
              vehicle_tf_msg.transform.translation.y);
    std::swap(vehicle_tf_msg.transform.rotation.x,
              vehicle_tf_msg.transform.rotation.y);
    vehicle_tf_msg.transform.translation.z =
        -vehicle_tf_msg.transform.translation.z;
...
void AirsimROSWrapper::append_static_lidar_tf(
  ...
  if (isENU_) {
    std::swap(lidar_tf_msg.transform.translation.x,
              lidar_tf_msg.transform.translation.y);
    std::swap(lidar_tf_msg.transform.rotation.x,
              lidar_tf_msg.transform.rotation.y);
    lidar_tf_msg.transform.translation.z =
        -lidar_tf_msg.transform.translation.z;
...
void AirsimROSWrapper::append_static_camera_tf(
    VehicleROS* vehicle_ros, const std::string& camera_name,
  ...
  tf2::Quaternion quat;
  quat.setRPY(camera_setting.rotation.roll, camera_setting.rotation.pitch,
  ...
  if (isENU_) {
    ...
    static_cam_tf_body_msg.transform.translation.z =
        -static_cam_tf_body_msg.transform.translation.z;
  ...
  mat_cam_optical.setValue(
      mat_cam_body.getColumn(1).getX(), mat_cam_body.getColumn(2).getX(),
    ...
    for (const auto& airsim_img_request_vehicle_name_pair :
      ...
      if (img_response.size() ==
        ...
        process_and_publish_img_response(
            img_response, image_response_idx,
            airsim_img_request_vehicle_name_pair.second);
      ...
      if (!vehicle_name_ptr_pair.second->lidar_pubs.empty()) {
        for (auto& lidar_publisher : vehicle_name_ptr_pair.second->lidar_pubs) {
          auto lidar_data = airsim_client_lidar_.getLidarData(
              lidar_publisher.sensor_name, vehicle_name_ptr_pair.first);
          sensor_msgs::PointCloud2 lidar_msg = get_lidar_msg_from_airsim(
              lidar_data, vehicle_name_ptr_pair.first);
...
sensor_msgs::ImagePtr AirsimROSWrapper::get_img_msg_from_response(
  ...
  if (is_vulkan_) img_msg_ptr->encoding = "rgb8";
...
void AirsimROSWrapper::process_and_publish_img_response(
  ...
  for (const auto& curr_img_response : img_response_vec) {
    ...
    // foolproofs us against render thread's async stuff, I hope. Ideally, we
    // should loop over cameras and then captures, and publish only one tf.
    publish_camera_tf(curr_img_response, curr_ros_time, vehicle_name,
...
void AirsimROSWrapper::publish_camera_tf(const ImageResponse& img_response,
  ...
  if (isENU_) {
    ...
    cam_tf_body_msg.transform.translation.z =
        -cam_tf_body_msg.transform.translation.z;
    cam_tf_body_msg.transform.rotation.z =
        -cam_tf_body_msg.transform.rotation.z;
  }
